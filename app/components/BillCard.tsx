...
export default function BillCard({ items, setItems, isScanning, handleFileUpload, t }: BillCardProps) {
    ...
    const saveEdit = (id: string) => {
        ...
        setItems(items.map(item => 
            item.id === id ? { ...item, name: editName.trim(), price: parseFloat(editPrice) } : item
        ))
        setEditingId(null)
    }
    ...
    const handleAddItem = () => {
        ...
        setNewItemName(''); setNewItemPrice(''); setAddError(''); setIsAddingItem(false)
    }
                        ...
                        {editingId === item.id ? (
                            <div className="space-y-2 animate-in fade-in zoom-in duration-200">
                                <div className="flex items-center gap-2 bg-brand-50/30 p-2 rounded-xl border border-brand/20">
                                    ...
                                    />
                                    <button onClick={() => saveEdit(item.id)} className="p-2 bg-brand text-white rounded-lg hover:bg-brand-dark transition-colors">
                                        <Check size={16}/>
                                    </button>
                                    <button onClick={() => setEditingId(null)} className="p-2 bg-slate-200 text-slate-500 rounded-lg"><X size={16}/></button>
                                </div>
                                {editError && <p className="text-xs text-red-500 flex items-center gap-1 ml-2"><AlertCircle size={12}/> {editError}</p>}
                            </div>
                        ) : (
                            <div className="flex justify-between items-center p-3 bg-slate-50 hover:bg-slate-100 transition-all rounded-xl border border-transparent hover:border-brand/10 group">
                                ...
                                <div className="flex items-center gap-3">
                                    <span className="text-brand font-bold" onClick={() => startEditing(item)}>
                                    ...
                                    <div className="flex items-center border-l border-slate-200 ml-2 pl-2 gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => startEditing(item)} className="text-slate-300 hover:text-brand p-1"><Edit3 size={16} /></button>
                                        <button onClick={() => handleDeleteItem(item.id)} className="text-slate-300 hover:text-red-500 p-1"><Trash2 size={16} /></button>
                                    </div>
                                </div>
                            </div>
                        )}
            ...
            {isAddingItem ? (
                <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-3 animate-in slide-in-from-top-2">
                    ...
                    <div>
                        <input
                            ...
                            placeholder={ t.bill.placeholderPrice }
                            value={newItemPrice}
                            onChange={(e) => {setNewItemPrice(e.target.value); setAddError('')}}
                    ...
                    <div className="flex gap-2 pt-1">
                        ...
                        <button 
                            onClick={() => {setIsAddingItem(false); setAddError('')}} 
                ...
                <div className="flex gap-3">
                    <div className="relative flex-1">
                        ...
                        <button className={`w-full py-3 flex items-center justify-center gap-2 rounded-xl font-bold border-2 transition-all ${isScanning ? 'bg-slate-50 text-slate-400 border-slate-100 cursor-not-allowed' : 'bg-brand-50 text-brand border-brand hover:bg-brand hover:text-white shadow-sm shadow-brand/10'}`}>
                            {isScanning ? <span className="animate-pulse">{ t.bill.scanning }</span> : <><Camera size={20} />{ t.bill.scan }</>}
                        </button>
                    </div>
                    <button 
